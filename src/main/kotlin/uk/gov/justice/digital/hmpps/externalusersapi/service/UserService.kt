...
class UserService(
  ...
  private val manageUsersApiService: ManageUsersApiService,
  ...
  suspend fun enableUserByUserId(userId: String) {
    userRepository.findById(UUID.fromString(userId))?.let { user ->
      ...
      user.email?.let { manageUsersApiService.sendEnableEmail(user, authenticationFacade.getUsername()) }
      log.info("User {} enabled email sent.", user)
      telemetryClient.trackEvent("AuthUserEnabled", mapOf("username" to user.name, "admin" to authenticationFacade.getUsername()), null)
